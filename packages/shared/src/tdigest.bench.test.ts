...
describe('TDigest Benchmarks', () => {
  ...
  test('addCentroid', {tags: ['bench']}, async () => {
    await bench('addCentroid', () => {
  ...
  test('addCentroidList', {tags: ['bench']}, async () => {
    await bench('addCentroidList', () => {
      const centroids = normalData.map(d => new Centroid(d, 1));
      const td = new TDigest(1000);
  ...
  test('quantile', {tags: ['bench']}, async () => {
    await bench('quantile', () => {
      const td = new TDigest(1000);
      for (const x of normalData) {
        td.add(x, 1);
      }
